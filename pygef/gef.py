...
#    @staticmethod
#    def parse_soil_color(data_rows_soil):
#        soil_color_col = ['soil_color']
#        lst = []
#        for row in data_rows_soil:
#            for i in range(len(row)):
#                if i == 1:
#                    if utils.is_color(row[i]) is True:  # check if it is a colour
#                        color = utils.create_color(row[i])
#                    else:
#                        color = "color not defined"
#                    lst.append(color)
#        df_soil_color = pd.DataFrame(lst, columns=soil_color_col)
#        return df_soil_color
...
#    @staticmethod
#    def parse_data_soil_type(data_s_rows, path, type_gef):
#        if path:
#            gef = re.search(r'gef', path.lower())
...
#                lst = []
#                df_soil_type = pd.DataFrame([])
#                for row in data_s_rows:
#                    soil_type = utils.create_soil_type(row)
#                    # append the soil type to the empty data frame
