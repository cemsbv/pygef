...
class Robertson:
    def __init__(self, path=None):
        ...
        # qt
        if gef.net_surface_area_quotient_of_the_cone_tip is not None and 'qc' \
                in gef.df.columns and 'u2' in gef.df.columns:
            qt = qc + gef.df['u2'] * (1 - gef.net_surface_area_quotient_of_the_cone_tip)
        else:
            qt = qc

        # calculation of sigma_v and u
        u = self.hydrostatic_water_pressure(self.water_level, gef.df['depth'])
        soil_type_robertson = []
        Ic = []
        sig0 = []
        series_Qt = []
        ...
        self.df_complete = pd.concat([gef.df, df_u, df_robertson, df_Qt, df_Fr], axis=1, sort=False)
