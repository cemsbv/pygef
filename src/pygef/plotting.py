...
if not _has_matplotlib:
    ...
    def plot_cpt():
        raise ImportError("matplotlib is required to create a plot")
else:
    ...
    def plot_cpt(
        ...
        if data.groundwater_level is not None:
            value = (
                data.groundwater_level_offset if use_offset else data.groundwater_level
            )
            p11 = ax.axhline(
                value,
        ...
        if data.predrilled_depth is not None:
            value = (
                data.predrilled_depth_offset if use_offset else data.predrilled_depth
            )
            p12 = ax.axhline(
                value,
